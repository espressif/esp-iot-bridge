...
static void esp_reboot_count_erase_timercb(void *priv)
{
    ...
    esp_storage_set(REBOOT_RECORD_KEY, &g_reboot_record, sizeof(qcloud_reboot_record_t));
...
static esp_err_t esp_reboot_unbroken_init()
{
    ...
    esp_storage_init();
    esp_storage_get(REBOOT_RECORD_KEY, &g_reboot_record, sizeof(qcloud_reboot_record_t));
    ...
    err = esp_storage_set(REBOOT_RECORD_KEY, &g_reboot_record, sizeof(qcloud_reboot_record_t));
...
bool esp_reboot_is_exception(bool erase_coredump)
{
    ...
    ESP_ERROR_RETURN(!coredump_part, false, "<%s> esp_partition_get fail", esp_err_to_name(ret));

    ret = esp_partition_read(coredump_part, 0, &coredump_len, sizeof(ssize_t));
    ESP_ERROR_RETURN(ret, false, "<%s> esp_partition_read fail", esp_err_to_name(ret));
    ...
    if (erase_coredump) {
        ...
        ESP_ERROR_RETURN(ret, false, "<%s> esp_partition_erase_range fail", esp_err_to_name(ret));
    }
