...
| 芯片     |  ESP-IDF Release/v5.2  |  ESP-IDF Release/v5.3  |  ESP-IDF Release/v5.4  |  ESP-IDF Release/v5.5  |  ESP-IDF Release/v6.0  |  ESP-IDF Release/v6.1  |
| :------- | :--------------------: | :--------------------: | :--------------------: | :--------------------: | :--------------------: | :--------------------: |
...
<img src="docs/_static/esp_iot_bridge.png" alt="esp_iot_bridge" style="zoom:80%;" />

根据接口的不同组合可以实现多种功能，如下表：

|                     | Softap        | 以太网接口     | USB 接口      | SPI/SDIO | Bluetooth LE 接口     | Thread 接口       |
| ------------------- | ------------- | ------------ | ------------ | --------- | -------------------- | ---------------- |
| **Station**         | Wi-Fi 路由器   | Wi-Fi 路由器  | 无线网卡       | 无线网卡  | Bluetooth LE 边界路由器 | Thread 边界路由器   |
| **以太网**           | Wi-Fi 路由器   | 暂不支持       | 有线网卡      | 有线网卡   | Bluetooth LE 边界路由器 | Thread 边界路由器   |
| **SPI/SDIO**        | MCU Wi-Fi 桥接 | MCU 以太网桥接 | MCU USB 桥接 | 暂不支持   | 暂不支持                | 暂不支持            |
| **Cat.1 4G (UART)** | 4G 路由器      | 4G 路由器     | 4G 网卡       | 4G 网卡   | Bluetooth LE 边界路由器 | Thread 边界路由器   |
| **Cat.1 4G (USB)**  | 4G 路由器      | 4G 路由器     | 不支持        | 4G 网卡   | Bluetooth LE 边界路由器  | Thread 边界路由器   |

备注：

- **第一列的 Station、以太网、Cat.1 4G (UART/USB)、以及 SPI/SDIO 是连接到互联网的接口。**

- **第一行的 Softap、以太网接口、USB 接口、SPI/SDIO 接口、Bluetooth LE 接口、以及 Thread 接口是为其它设备提供上网功能的接入接口。**

- **当前版本以太网、SPI/SDIO 同一时刻无法同时作为 external netif（连接外部网络）和 data forwarding netif（为其他设备转发数据）工作。** 不过 SPI/SDIO 可同时启用两者以分别创建 WAN/LAN netif，运行时通过 `esp_bridge_spi_set_netif_type()` / `esp_bridge_sdio_set_netif_type()` 切换当前角色（`IOT_BRIDGE_NETIF_WAN` 或 `IOT_BRIDGE_NETIF_LAN`）。以太网另可启用 ``Ethernet acts as WAN or LAN automatically`` 根据网络环境自动切换角色。

可以总结出上表主要涉及以下几种应用场景：Wi-Fi 路由器、4G 路由器、4G 网卡、无线网卡、有线网卡、MCU Wi-Fi 桥接、MCU 以太网桥接、MCU USB 桥接、Bluetooth LE 边界路由器和 Thread 边界路由器。ESP 芯片对这些场景的支持情况如下表所示：

| ESP 设备 | Wi-Fi 路由器            | 4G 路由器               | 4G 网卡                               | 无线网卡                              | 有线网卡                              | MCU Wi-Fi 桥接                        | MCU 以太网桥接                         | MCU USB 桥接                          | Bluetooth LE 边界路由器 | Thread 边界路由器 |
| -------- | ---------------------- | ---------------------- | ------------------------------------ | ------------------------------------ | ------------------------------------ | ------------------------------------ | ------------------------------------- | ------------------------------------ | ----------------------- | ---------------- |
...
- 自 **iot_bridge v1.0.0** 起，USB 功能需要使用 **ESP-IDF v5.1.4 或更高版本**。对于使用 **ESP-IDF 5.0-5.1.3** 的系统：
    ...
    | **iot_bridge ≥1.0.0** | **ESP-IDF 5.0-5.1.3** | ❌ 不支持 | ❌ 不支持   | 升级 ESP-IDF **或** 降级至 iot_bridge 0.11.9 |
...
- 除了 C2，单个设备最多支持 15 个 子设备同时连接（具体参考 [AP 基本配置](https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32/api-guides/wifi-driver/overview.html#ap) 中 max_connection），多个子设备共享带宽。
...
- 成功获取到 ESP-IDF 之后，请将 ESP-IDF 版本切换到 `release/v5.2`  及其以上版本。
...
- 是否支持选择多个网络数据转发接口来给不同设备提供网络功能？
    ...
    | IDF Version               |          | 备注                          |
    | ------------------------- | -------- | ---------------------------- |
    | ESP-IDF Release/v5.2-v6.1 | **支持** | 目前不能同时选择 SDIO 和 SPI 接口 |
...
<img src="docs/_static/gpio_map.png" alt="gpio_map" style="zoom:80%;" />
