 ...
 */
static void spi_low_level_init(struct netif *netif)
{
...
static void spi_driver_free_rx_buffer(void *h, void* buffer)
{
    if (buffer) {
        free(buffer);
    }
...
static esp_err_t spi_io_transmit(void *h, void *buffer, size_t len)
{
    // send data to driver
...
static esp_err_t spi_io_transmit_wrap(void *h, void *buffer, size_t len, void *netstack_buf)
{
    return spi_io_transmit(h, buffer, len);
}

static esp_netif_driver_ifconfig_t spi_driver_ifconfig = {
    .driver_free_rx_buffer = spi_driver_free_rx_buffer,
    .transmit = spi_io_transmit,
    .transmit_wrap = spi_io_transmit_wrap,
    .handle = "SPI" // this IO object is a singleton, its handle uses as a name
};

const struct esp_netif_lwip_vanilla_config spi_netstack_config = {
    .init_fn = spi_init,
    .input_fn = spi_input
};
